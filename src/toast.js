...
export function showToast({
  ...
  progressBarColor = '#4caf50' // Color personalizado para la barra de progreso
}) {
  const icons = {
    alert: `
  ...
  toast.className = `jrt-toast jrt-toast-${position} jrt-toast-${type}`;
  ...
  toast.innerHTML = `
    <div class="jrt-toast-icon">${iconHTML}</div>
    <div class="jrt-toast-message">${message}</div>
  `;

  // Crear barra de progreso
  const progressBar = document.createElement('div');
  progressBar.className = 'jrt-toast-progress';
  progressBar.style.animationDuration = `${duration}ms`;
  progressBar.style.backgroundColor = progressBarColor; // Aplicar color personalizado

  toast.appendChild(progressBar);
  document.body.appendChild(toast);

  // Ocultar y eliminar Toast
  setTimeout(() => {
